...
bool MyBLE2::isPacketValid(byte *packet) // check if packet is valid
{
    // TRACE;
    if (packet == nullptr)
    {
...
void MyBLE2::notifyCallback(BLERemoteCharacteristic *pBLERemoteCharacteristic, uint8_t *pData, size_t length, bool isNotify) // this is called when BLE server sents data via notofication
{
    // TRACE;
...
void MyBLE2::printCellInfo() // debug all data to uart
{
    if (!MyLOG::DISABLE_LOGD)
    {
        commSerial.println();
...
void MyBLE2::sendCommand(uint8_t *data, uint32_t dataLen)
{
    // TRACE;
...
bool MyBLE2::reconnectTmeout(int currentTime)
{
    if ((currentTime - lastDisconnect) >= reconnectIntervalMs)
    {
        LOGD("MyBLE2", "millis() - lastDisconnect: " + String(currentTime) + " - " + String(lastDisconnect) + " >= reconnectIntervalMs: " + String(reconnectIntervalMs));
        return true;
    }
    else
        return false;
